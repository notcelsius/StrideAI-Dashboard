import { parseGPSPoints } from "./hubComputation";

export async function fetchAndParseCSVs(exportFiles, onProgress, maxConcurrency = 6) {
  const withUrls = exportFiles.filter((f) => f.downloadUrl);
  if (withUrls.length === 0) return [];

  const allPoints = [];
  let completed = 0;
  let failed = 0;

  for (let i = 0; i < withUrls.length; i += maxConcurrency) {
    const batch = withUrls.slice(i, i + maxConcurrency);
    const results = await Promise.allSettled(
      batch.map(async (file) => {
        const res = await fetch(file.downloadUrl);
        if (!res.ok) {
          if (res.status === 403) throw new Error("EXPIRED");
          throw new Error(`HTTP ${res.status}`);
        }
        // Gzipped exports (.csv.gz) are stored with Content-Type: application/gzip,
        // not Content-Encoding: gzip, so the browser won't auto-inflate. Detect the
        // suffix/type and decompress with the native DecompressionStream. Plain .csv
        // still flows through res.text(), so legacy uploads keep working.
        const isGz =
          (file.fileName || file.fileKey || "").toLowerCase().endsWith(".gz") ||
          (file.contentType || "").toLowerCase().includes("gzip");
        if (!isGz) return res.text();
        const inflated = res.body.pipeThrough(new DecompressionStream("gzip"));
        return new Response(inflated).text();
      })
    );

    for (const result of results) {
      completed++;
      if (onProgress) onProgress(completed, withUrls.length);

      if (result.status === "fulfilled") {
        const points = parseGPSPoints(result.value);
        for (const p of points) allPoints.push(p);
      } else {
        failed++;
        if (result.reason?.message === "EXPIRED") {
          throw new Error("Presigned URLs have expired. Please reload CSV exports.");
        }
      }
    }
  }

  // Don't let download failures masquerade as "no data". A CORS-misconfigured
  // bucket or network error rejects every fetch; surface that instead of
  // silently returning an empty set.
  if (allPoints.length === 0 && failed > 0) {
    throw new Error(
      `Could not download any of the ${failed} CSV file${failed !== 1 ? "s" : ""} (likely a CORS or network issue with the storage bucket).`
    );
  }

  allPoints.sort((a, b) => a.timestamp - b.timestamp);
  return allPoints;
}
