...
function handleUnauthorized(response) {
  if (response.status === 401 && typeof window !== "undefined") {
    const { clearSession } = require("@/lib/cognitoAuth");
    clearSession();
...
export async function linkPatientSubject(session, patientSub, subjectId, projectId) {
  const res = await fetch(`${API_BASE_URL}/admin/subject-links`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders(session) },
    body: JSON.stringify({ patientSub, subjectId, projectId }),
  });
  if (!res.ok) {
...
export async function createSubject(session, subjectId, projectId, participantName) {
  const res = await fetch(`${API_BASE_URL}/admin/subjects`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders(session) },
    body: JSON.stringify({ subjectId, projectId, participantName }),
  });
  if (!res.ok) {
    const payload = await res.json().catch(() => ({}));
    throw new Error(payload.error || `Request failed with ${res.status}`);
...
export async function createEnrollmentCode(session, subjectId, projectId, participantName) {
  const res = await fetch(`${API_BASE_URL}/admin/enrollment-codes`, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...authHeaders(session) },
    body: JSON.stringify({ subjectId, projectId, participantName }),
  });
