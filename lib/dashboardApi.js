...
// Patient-set named-location labels (Home / Clinic / etc.). These are the
// current full set for the subject, not date-ranged — labels persist across
// sessions. Returns { subjectId, projectId, locations: [{ id, label, latitude,
// longitude, createdDate, updatedAt }] }.
export async function getSubjectLocations(session, subjectId, projectId = "") {
  const query = new URLSearchParams(projectId ? { projectId } : {});
  const suffix = query.toString() ? `?${query.toString()}` : "";
  return fetchJson(`/subjects/${subjectId}/locations${suffix}`, session);
}
