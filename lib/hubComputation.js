...
// A value of -1 means accuracy is unknown/legacy (CSVs without the accuracy_m
// column) and is always kept — this filter only removes known-bad fixes.
...
export function parseGPSPoints(csvText) {
  ...
  const accIdx = header.indexOf("accuracy_m");
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const accVal = accIdx !== -1 ? parseFloat(cols[accIdx]) : NaN;
    const accuracy = Number.isFinite(accVal) ? accVal : -1;
    // Skip known-bad fixes; -1 (unknown/legacy) is never > MAX_ACCURACY_M, so it's kept.
    if (accuracy > MAX_ACCURACY_M) continue;
